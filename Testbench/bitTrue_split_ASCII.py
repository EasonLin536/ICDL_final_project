...
def Padding(orig_img, padnum=1, printPad=False, noPad=False, file=False):
    ...
    if file:
        ...
        for row in img:
            for i in range(4):
                pixel_in[0].append(ToASCII(row[5*i]))
                pixel_in[1].append(ToASCII(row[5*i+1]))
                pixel_in[2].append(ToASCII(row[5*i+2]))
                pixel_in[3].append(ToASCII(row[5*i+3]))
                pixel_in[4].append(ToASCII(row[5*i+4]))
...
def main():
    ...
    in_fname = sys.argv[1]
    out_dir = sys.argv[2]
    if os.path.isdir(f"pattern/{out_dir}") is False:
        os.makedirs(f"pattern/{out_dir}")

    orig_img = Image.open(in_fname)
    ...
    for i in range(5):
        with open(f"pattern/{out_dir}/pixel_in" + str(i) + ".dat", 'w') as f:
            f.write(''.join(pixel_in[i]))

    with open(f"pattern/{out_dir}/out_golden.dat", 'w') as f:
